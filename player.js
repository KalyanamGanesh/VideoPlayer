var playlistData = [
  ...
  var player_wrapper=document.getElementById('player-wrapper');
  player_wrapper.innerHTML=`<iframe id="video-player" src="https://player.vimeo.com/video/190062231" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
              <div>
...
var playlist_wrapper=document.getElementById('playlist-wrapper');
for(var i=0;i<playlistData.length;i++){
    if(i==0){
        playlist_wrapper.innerHTML += `<div id="card${i+1}" class="playlist-card active-card" onclick="AddActiveCard(${i+1})"> <img class="thumbnail" src="${playlistData[i].thumbnail}" /><h3 class="video-card-title">${playlistData[i].title}</h3></div>`;
    }else{
        playlist_wrapper.innerHTML += `<div id="card${i+1}" class="playlist-card" onclick="AddActiveCard(${i+1})"> <img class="thumbnail" src="${playlistData[i].thumbnail}" /><h3 class="video-card-title">${playlistData[i].title}</h3></div>`;
    }
}
function AddActiveCard(n){
    var prev=$('.active-card');
    prev.removeClass('active-card');
    var clickedEle=$(`#card${n}`);
    clickedEle.addClass('active-card');
    player_wrapper.innerHTML=`<iframe id="video-player" src="https://player.vimeo.com/video/${videoPlaySectionData[n-1].vimeoId}" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
              <div>
                  <div id="video-actions">
                      <p><span id="views-count">${videoPlaySectionData[n-1].views/1000}k</span> views</p>
                  ...
                  <h3 id="video-title">${videoPlaySectionData[n-1].title}</h3>
                  <p id="video-description">${videoPlaySectionData[n-1].description}</p>
              </div> `
